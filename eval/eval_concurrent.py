...
def generate(args, prompt_batch, sampling_params, q, device):
    ...
    q.put({"device":device, "completions": completions})
...
def infer(args):
    ...
    for cur_generation_epoch in range(generation_epoch):
        ...
        print(f"Total prompts: {len(prompt_batch)}")
        per_gpu_size = len(prompt_batch) // len(available_gpus)
        # reminder = len(prompt_batch) % len(available_gpus)
        # if reminder > 0:
        #     per_gpu_size += 1
        ...
        for i in range(len(available_gpus)):            
            if i == len(available_gpus) - 1:
                per_gpu_batch = prompt_batch[i*per_gpu_size:]
            else:
                per_gpu_batch = prompt_batch[i*per_gpu_size : (i+1)*per_gpu_size]

            print(f"\n\n ##########  for GPU{i}: data size: {len(per_gpu_batch)}\n\n")

            process = multiprocessing.Process(target=generate, args=(args, per_gpu_batch, sampling_params, q, available_gpus[i]))
        ...
        for _ in processes:
            result = q.get()
            all_results[result["device"]] = result["completions"]
        ...
        for device in available_gpus:
            completions += all_results[device]
